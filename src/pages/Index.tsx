...
const Index = () => {
  ...
  return (
    <div className="min-h-screen flex bg-[#2A2317]">
      ...
      <main className="flex-1 flex flex-col min-h-screen">
        {/* Hero Section */}
        <section className="relative flex flex-col md:flex-row min-h-[500px] md:min-h-[75vh] w-full items-center overflow-hidden bg-[#19150e] border-b border-[#c4975a]/10">
          {/* Hero Visual - Left (Image) */}
          <div className="relative w-full md:w-1/2 h-[340px] md:h-full flex-shrink-0 flex items-end justify-center">
            <div className="absolute inset-0 bg-cover bg-center grayscale-0 md:grayscale animate-slow-zoom-hero" style={{
              backgroundImage: "url('https://images.unsplash.com/photo-1581092795360-fd1ca04f0952?w=1080&h=1080&fit=crop&crop=face')"
            }}>
              <div className="absolute inset-0 bg-gradient-to-tr from-[#19150e]/80 to-transparent"></div>
            </div>
            {/* Overlayed music notes (visual touch) */}
            <div className="absolute bottom-8 right-8 text-3xl text-[#C4975A]/70 animate-float select-none hidden md:block">🎵</div>
          </div>

          {/* Hero Content - Right */}
          <div className="relative z-10 w-full md:w-1/2 flex flex-col justify-center items-start p-8 md:pl-12 lg:pl-24">
            <h1 className="text-[#F5E6D3] text-5xl sm:text-6xl lg:text-7xl font-extrabold font-serif mb-1 tracking-tight">DIMITRI</h1>
            <h2 className="text-[#C4975A] text-xl md:text-2xl font-light italic font-serif mb-3">Singer • Songwriter • Producer</h2>
            <p className="text-[#D4B896] text-base md:text-lg max-w-xl font-serif mb-6">
              Crafting rustic folk melodies and modern soundscapes. Welcome to the field—where stories, soil, and sound converge.
            </p>
            <div className="w-full mb-6">
              <AudioPlayer />
            </div>
            <div className="flex flex-wrap gap-4">
              <a href="#listen-now" className="bg-[#C4975A] hover:bg-[#8B7355] text-[#F5E6D3] font-semibold py-3 px-6 rounded-full shadow font-serif text-base transition-all">Listen Now</a>
              <a href="#events" className="bg-transparent border border-[#F5E6D3]/30 hover:border-[#F5E6D3] text-[#F5E6D3] font-semibold py-3 px-6 rounded-full font-serif text-base transition-all">Next Tour</a>
              <a href="#contact" className="bg-transparent border border-[#C4975A]/30 hover:border-[#C4975A] text-[#C4975A] font-semibold py-3 px-6 rounded-full font-serif text-base transition-all">Contact</a>
            </div>
          </div>
        </section>
        
        {/* Main content (existing sections, moved below hero) */}
